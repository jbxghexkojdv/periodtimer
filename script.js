...
const scheduleData = {
    ...
    normal: [
             ...
             /*Teleport*/"13:15", // Lu 8
              ...
              ],
              {
                ...
                tuesday: [// Start, End
                  "07:55", "09:28", // P1/2 THS BLOCK
                  "09:33", "10:20", // P3   THS BLOCK
                  "10:25", "11:09", // TrTi THS BLOCK
                  "11:14", "11:44", // Lunc THS BLOCK
                  "11:49", "13:22", // P4/5 THS BLOCK
  ...
  names: {
    normal: [
              [ // 6
                ...
                ["6-7", "6-7", "6-7"],
                ["7<sup>th</sup> period", "7<sup>a</sup> hora", "7<sup>e</sup> cours"],
                ["7-8", "7-8", "7-8"],
                ["8<sup>th</sup> period", "8<sup>a</sup> hora", "8<sup>e</sup> cours"]
              ],
              [ // 7
                ["AB", "BC", "BC"],
                ["AB-1"],
              ...
              ],
              {
                ...
                tuesday: [ // THS EVEN BLOCK
                  ["2<sup>nd</sup> period", "2<sup>a</sup> hora", "2<sup>e</sup> cours"],
                  ["2-3", "2-3", "2-3"],
                  ["3<sup>rd</sup> period", "3<sup>a</sup> hora", "3<sup>e</sup> cours"],
                  ["3-TT", "3-TT", "3-TT"],
                  ["Trojan Time", "Trojan Time", "Trojan Time"],
                  ["TT-Lunch", "TT-Almuerzo", "TT-Déjeuner"],
                  ["Lunch", "Almuerzo", "Déjeuner"],
  ...
  passing: {
    ...
    assembly: [
 // if you want this data you can live peacefully and frolic in the fields without it
    ]
  }
};
...
const userStyling = 
{
  gradechange()
  {
    ...
    if (settings.grade < 0)
    {
      settings.grade += scheduleData.times.normal.length;
    }
    settings.grade %= scheduleData.times.normal.length;
    mainModule.setSettings(settings);
    update("grade");
...
function updateTimer(timesIn, periodsIn, flash = 0, passing = [])
{
  ...
  for (let i = 0; i + 1 < timesIn.length; i++)
  {
    if ((now > time.ofDay(timesIn[i])) && (now < time.ofDay(timesIn[i + 1])))
    {
      ...
      if (flash > 0 && passing.includes(i) && time.ofDay(timesIn[i + 1])-now < flash * 1000 && Math.round(Date.now()/1000) % 2 == 1)
      {
 ...
 * 
 * @param {string} string 
 * @param {number} craziness Code point can go this many in either direction, decimal after adds a fraction of the chance
 */
function glitch(string, craziness)
{
  craziness += 0.5;
  let retval = "";
  for (let i of string)
  {
    let newCode = -1;
    while (newCode < 0x1f || (newCode > 0x7e && newCode < 0xa0))
    {
      newCode = i.codePointAt(0) + Math.round((Math.random()*craziness*2)-craziness);
    }
    retval += String.fromCodePoint(newCode);
  }
  return retval;
}
